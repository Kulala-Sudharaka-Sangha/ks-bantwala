...
export interface ZoneCommitteeMembers {
  id: string;
  teamName: string;
  aboutTeam: string;
  members: Member[];
...
interface DataState {
  zoneCommitteeMembers: ZoneCommitteeMembers[];
  loading: boolean;
  error: string | null;
  cached: boolean;
}
...
export const fetchZoneCommitteeMembers = createAsyncThunk<
  ZoneCommitteeMembers[],
  void,
  { state: { zoneCommitteeMembers: DataState } }
>(
  "data/fetchZoneCommitteeMembers",
...
const dataSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchZoneCommitteeMembers.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(fetchZoneCommitteeMembers.fulfilled, (state, action) => {
        state.zoneCommitteeMembers = action.payload;
        state.loading = false;
        state.cached = true;
      })
      .addCase(fetchZoneCommitteeMembers.rejected, (state, action) => {
        state.loading = false;
